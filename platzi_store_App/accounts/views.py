...
def register_view(request):
    """
    Vista para el registro de usuarios - Versión corregida
    """
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(f"Intentando registrar usuario: {user_data['username']}")
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    if existing_user:
                        ...
                        messages.success(
                            request,
                            f'¡Registro exitoso! Bienvenido {existing_user.first_name}. Ya puedes iniciar sesión.'
                        )
                    else:
                        ...
                        try:
                            ...
                            print(f"Usuario creado exitosamente: {new_user.username}")
                            messages.success(
                                request,
                                f'¡Registro exitoso! Bienvenido {new_user.first_name}. Tu cuenta ha sido creada.'
                            )
                        except Exception as local_error:
                            print(f"Error creando usuario local: {local_error}")
                            messages.success(
                                request,
                                'Registro exitoso en el servidor. Ya puedes iniciar sesión.'
                            )
                    
                    return redirect('accounts:login')

                elif response.status_code == 400:
                    print(f"Error 400 en API: {response.text}")
                    try:
                    ...
                    print(f"Error del servidor API: {response.status_code}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if user and user.is_active:
                        ...
                        next_url = request.GET.get('next', 'products:list_products')
